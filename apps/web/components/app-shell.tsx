...
export function AppShell({ children, title, eyebrow = "Operations workspace" }: { children: ReactNode; title: string; eyebrow?: string }) {
  ...
  return <div className="min-h-screen bg-[#f3f5f8] text-slate-950">
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col bg-[#08111f] text-slate-200 transition-transform md:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
      ...
      <div className="border-t border-white/10 p-4"><div className="mb-3 rounded-xl bg-white/5 p-3"><div className="flex items-center gap-2 text-xs text-emerald-300"><Activity size={14} /> UCI public dataset</div><p className="mt-1 text-xs leading-5 text-slate-400">External funding and payout confirmations run through the development provider.</p></div><button onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-slate-300 hover:bg-white/10 hover:text-white"><LogOut size={18} /> Sign out</button></div>
    </aside>

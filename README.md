...
All screenshots below were captured from the application running locally against the UCI Online Retail public dataset demo seed.
...
The API container runs Alembic migrations and seeds the public dataset demo by default. Set `FINCORE_SEED_ON_START=false` to skip seeding.
...
FinCore Ledger uses the [UCI Online Retail dataset](https://archive.ics.uci.edu/dataset/352/online%2Bretail) to demonstrate real invoice-shaped retail money movement through the wallet, merchant payment, refund, withdrawal, webhook, ledger, and reconciliation workflows.

- Dataset: Online Retail.
- Publisher: UCI Machine Learning Repository.
- License: Creative Commons Attribution 4.0 International (CC BY 4.0).
- Citation: Chen, D. (2015). Online Retail [Dataset]. UCI Machine Learning Repository. https://doi.org/10.24432/C5BW33.
- Committed subset: `data/sample/uci_online_retail_payments.json`, a 19 KB processed sample with 18 invoice-derived payments and 1 cancellation-derived refund.
- Practical sample source: IBM `customer_pos_analytics` `Online Retail Sample.csv`, downloaded by `scripts/prepare_uci_online_retail_sample.py`.
- Raw downloads: stored under ignored `data/raw/`.

Prepare or refresh the processed sample:

```bash
python scripts/prepare_uci_online_retail_sample.py
```

Seed the application after migrations:
...
The seed creates public-dataset-labeled organizations, GBP wallets, deposits, captured merchant payments, one refund, a merchant payout review, a webhook endpoint, balanced journals, and a reconciliation run. It is idempotent and safe to run repeatedly against the same development database.

The dataset does not represent FinCore customers, production money movement, private bank records, payment cards, credentials, or a regulated deployment. It is retail invoice data mapped into the FinCore ledger model for engineering demonstration. More details are in [docs/DATASET.md](docs/DATASET.md).

All seeded identities use the development-only password `FinCore-Dev-2026!`.
...
curl -X POST http://localhost:8000/api/v1/transfers \
  ...
  -d '{
    ...
    "amount":25000,
    "currency":"GBP",
    "reference":"UCI-MANUAL-TRANSFER-1007",
    "description":"Public dataset invoice settlement"
  }'
```
...
- [Public dataset demo](docs/DATASET.md)
...
FinCore Ledger is built with the open-source libraries listed in the Python and npm lock/configuration files. The included public dataset demo is attributed in [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md); seeded organizations and users are development-only wrappers around the public invoice subset.
